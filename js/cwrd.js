...
class CodewordArray extends Uint8ClampedArray {
	...
	encode (cws, datarr, counterref, offset) {
		...
		for (i = 0; i < datarr.length; i++) {
			...
			if (datarr[i].encoding !== "enddata" && chars === "") throw new Error("You've tried to encode nothing!");

			if (_encoding !== datarr[i].encoding) switch (datarr[i].encoding) { // ADDING ECI INDICATOR AND CODE
				case "latin1":
					buff = (buff << 4) + 0b0111;
					buff = (buff << 8) + 3;
					k += 12;
			...
			for (j = 0; j < 4 && k >= 8; j++) { // DISTRIBUTION OF MODE BITS
				k -= 8;
				cws[c++] = buff >> k;
				buff %= 1 << k;
			}
	...
	static uninterleave (cws, g1rows, g2rows, g1cols) {
		if (cws instanceof CodewordArray && cws.length === (g1rows * g1cols) + (g2rows * (g1cols + 1))) {
			...
			for (let x = 0; x < g1rows + g2rows; x++) { // interleaving
